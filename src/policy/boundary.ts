...
export function authorize(tool: string, mode: string): void {
  if (!['Review','Workspace','Full access','Custom'].includes(mode) || (!allowed.has(tool) && !(mutations.has(tool) && ['Workspace','Full access'].includes(mode)))) throw new Error(`Tool denied in ${mode} mode.`);
}

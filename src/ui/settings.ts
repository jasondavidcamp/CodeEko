...
</style></head><body><nav aria-label="Settings sections"><p>LLM Runtime</p>${['Connection','Editor','Permissions','Validation','Diagnostics'].map(group => `<button data-group="${group}" aria-current="${group === 'Connection'}">${group}</button>`).join('')}</nav><main><h1 id="heading">Connection</h1><p class="subtitle">Configure your coding assistant. Changes save automatically.</p><div id="content"></div><p id="notice" role="status" aria-live="polite"></p></main>
<script nonce="${nonce}">

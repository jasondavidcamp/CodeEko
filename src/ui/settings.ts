  ...
  open(): void {
    ...
    const listener = panel.webview.onDidReceiveMessage(async message => {
      ...
      try {
        ...
        } else if (message?.type === 'clearPerformance') { performanceDiagnostics.clear(); send('Performance history cleared.');
...
</style></head><body><nav aria-label="Settings sections"><p>EKOD</p>${['Connection','Editor','Permissions','Validation','Diagnostics'].map(group => `<button data-group="${group}" aria-current="${group === 'Connection'}">${group}</button>`).join('')}<div id="version" aria-label="Installed EKOD version"></div></nav><main><h1 id="heading">Connection</h1><p class="subtitle">Configure your coding assistant. Changes save automatically.</p><div id="content"></div><p id="notice" role="status" aria-live="polite"></p></main>
<script nonce="${nonce}">
...
const description=document.createElement('p');description.className='description';description.textContent='Latest 100 API requests in this extension session, including repair requests. Times include network and server processing; they cannot distinguish proxy delay from model computation. No prompts, responses, endpoint addresses or keys are recorded. History resets on restart.';section.append(description);
...
const list=document.createElement('div');list.style.overflowX='auto';const table=document.createElement('table');table.style.width='100%';const header=document.createElement('tr');for(const label of ['Time','Request','Mode','Total','Headers','Result']){const th=document.createElement('th');th.textContent=label;header.append(th);}table.append(header);
for(const item of [...performanceReport.requests].reverse()){const tr=document.createElement('tr');for(const value of [new Date(item.at).toLocaleTimeString(),item.repair?'Format repair':item.operation,item.mode,item.elapsedMs===undefined?'In progress':(item.elapsedMs/1000).toFixed(2)+'s',item.headersMs===undefined?'—':(item.headersMs/1000).toFixed(2)+'s',item.outcome+(item.status?' (HTTP '+item.status+')':'')]){const td=document.createElement('td');td.textContent=value;tr.append(td);}table.append(tr);}list.append(table);if(!performanceReport.requests.length){const empty=document.createElement('p');empty.textContent='No requests recorded yet. Send a chat message, then refresh.';list.append(empty);}section.append(list);content.append(section);

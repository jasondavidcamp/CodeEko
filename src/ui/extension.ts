...
import { contained } from '../policy/boundary';
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  command('llmRuntime.open', () => open(context));
}
    ...
    try {
      if (message.type === 'new') {
        ...
        try {
          ...
          const summary = await runAgent(api, model, thread.messages, new ReadOnlyTools(index, ask), mode, controller.signal, text => {
            thread.activity.push({ at: new Date().toISOString(), event: text });
            thread.activity = thread.activity.slice(-500);
            send({ type: 'progress', text });
          });
          thread.messages.push({ role: 'assistant', content: summary }); thread.status = 'complete';
        } catch (e) {
          thread.status = controller.signal.aborted ? 'cancelled' : 'failed';
          thread.messages.push({ role: 'assistant', content: controller.signal.aborted ? 'Task cancelled. No repository files were changed.' : e instanceof Error ? e.message : 'Task failed.' });
...
<h2>LLM Coding Agent Runtime</h2><p id="boundary"></p><label for="threads">Conversation</label><select id="threads"></select><button id="new">New conversation</button><main id="messages" aria-live="polite"></main><p id="status" role="status"></p><label for="input">Message</label><textarea id="input" maxlength="8000" placeholder="Ask about this repository…"></textarea><button id="send">Send</button><button id="cancel">Cancel task</button>
...
window.addEventListener('message',event=>{const m=event.data;if(m.type==='progress'){el('status').textContent=m.text;return;}if(m.type!=='state')return;busy=m.busy;el('boundary').textContent=m.root+' • '+m.mode+' • Read-only release';el('threads').replaceChildren();for(const t of m.threads){const o=document.createElement('option');o.value=t.id;o.textContent=t.name;o.selected=t.id===m.thread.id;el('threads').append(o);}el('messages').replaceChildren();for(const msg of m.thread.messages){const a=document.createElement('article');a.textContent=(msg.role==='user'?'You':'Agent')+'\\n'+msg.content;el('messages').append(a);}for(const id of ['send','new','threads'])el(id).disabled=busy;el('cancel').disabled=!busy;if(!busy)el('status').textContent=m.thread.status;});vscode.postMessage({type:'ready'});</script></body></html>`;
}

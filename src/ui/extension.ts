...
async function client(context: vscode.ExtensionContext): Promise<GeminiClient> {
  ...
  return new GeminiClient(endpoint, key, config().get<number>('requestTimeout', 300000), fetch, config().get<string>('compatibilityMode', 'User message') === 'Standard' ? 'Standard' : 'User message');
}

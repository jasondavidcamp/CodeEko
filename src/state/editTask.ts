...
export class EditTask {
  ...
  async undo(signal: AbortSignal): Promise<void> {
    ...
    for (const change of changes) {
      ...
      if (change.before === null) {
        ...
        const temporary = path.join(path.dirname(current.full), `.codeeko-${randomUUID()}.tmp`);
  ...
  private async allowedPath(file: string): Promise<string> {
    if (excluded(file) || /(^|\/)(\.git[^/]*|\.codeeko-[^/]*|\.vscode)(\/|$)/i.test(file)) throw new TaskConflict(`Editing protected configuration or excluded content is not allowed: ${file}`);
  ...
  private async write(file: string, expected: string | null, bytes: Buffer, operation: 'patch' | 'create', signal: AbortSignal): Promise<void> {
    ...
    const temporary = path.join(path.dirname(full), `.codeeko-${randomUUID()}.tmp`);
    let fileMode = 0o644;

...
The established repository is `EKOD`. The extension ID and `llmRuntime.*` settings are retained as compatibility identifiers; use EKOD for all visible product names. Use the product name established in current metadata. Do not rename the package, commands, settings, or publisher as part of unrelated work.

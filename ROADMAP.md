...
- [x] Add an explicitly selected transport/compression comparison with twenty identical fresh-chat requests across VS Code fetch and an owned, reused PowerShell HttpClient, default/identity encoding, bounded response-header categories, UTC timestamps, native worker timings, configuration flags, cancellation and metadata-only reports. Fake transports and real Windows PowerShell 5.1/PowerShell 7 loopback checks verify payload equality, delayed/mislabeled SSE, gzip, failures, cancellation, cleanup and redaction. Normal chat settings and security checks remain unchanged.
- [ ] Run the paired transport/compression comparison on affected workstations and correlate slow request timestamps with available network/certificate-validation evidence. Investigate header-to-body stalls separately from pre-header waits; do not infer DNS, certificate, gateway or security-product responsibility from aggregate request timing alone.
...
- [x] Reduce fixed coding instructions through a compact core and per-run, action-selected guidance modules, without a greeting classifier, routing model call or history compaction. Fake-model regressions verify questions, short follow-ups, direct edits, guidance loading, format repairs and permission changes. Fresh Full access/User message `hey` measures 3,613 prompt characters versus 8,235 with the full protocol; this is a payload result, not a live speed or quality claim.
- [x] Extend the timing comparison with the frozen full-protocol baseline and current core, request-size medians and bounded in-memory agent workflows. Fake-provider checks verify call/repair counts, requested content, preserved developer comments, simulated automatic validation, false-completion detection, cancellation and metadata-only reports.
- [ ] Run the full/progressive comparison on affected workstations and evaluate prompt size, total model-call count, response quality and latency. Verify real editing, validation, native review/undo and ambiguous follow-ups after side questions/reload on representative repositories. Do not infer live quality or latency improvement from fake responses or simulated validation.

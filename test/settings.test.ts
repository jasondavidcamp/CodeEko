...
test('settings tab validates writes, reuses its panel and never exposes keys', async () => {
  ...
  const mock={ViewColumn:{Active:1},ConfigurationTarget:{Global:1},window:{createWebviewPanel(){panels++;return panel;}},commands:{executeCommand:async(name:string)=>{calls.push(name);}},workspace:{getConfiguration:()=>({get:(key:string,fallback:unknown)=>values[key]??fallback,update:async(key:string,value:unknown,target:number)=>{assert.equal(target,1);values[key]=value;}}),onDidChangeConfiguration(fn:typeof changed){changed=fn;return disposable;}}};
  ...
  await receive({type:'setKey'});await receive({type:'export'});assert.deepEqual(calls,['llmRuntime.setKey','llmRuntime.exportStartupDiagnostics']);

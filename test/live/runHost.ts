...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!result.extensionActivation || !result.live?.repositoryUnchanged || !result.editing?.preexistingWorkPreserved || !result.nativeDiffsOpened || !result.validation?.repaired) throw new Error('Extension host did not report completed checks.');

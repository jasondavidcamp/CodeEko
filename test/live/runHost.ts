...
async function main(): Promise<void> {
  ...
  const executable = process.env.CODEEKO_VSCODE_EXECUTABLE ?? await downloadAndUnzipVSCode({
    version: process.env.CODEEKO_VSCODE_VERSION ?? 'stable',
    cachePath: path.join(os.tmpdir(), 'codeeko-vscode-cache')
  });
  const temp = await fs.mkdtemp(path.join(os.tmpdir(), 'codeeko-host-'));
  ...
  const env = { ...process.env, CODEEKO_HOST_REPORT: report, CODEEKO_HOST_UI_ONLY: uiOnly ? '1' : '0' }; delete (env as NodeJS.ProcessEnv).ELECTRON_RUN_AS_NODE;
  try {

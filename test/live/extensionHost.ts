...
export async function run(): Promise<void> {
  ...
  for (const command of ['codeeko.open', 'codeeko.setKey', 'codeeko.selectModel']) assert.ok(commands.includes(command));
  assert.equal(vscode.workspace.getConfiguration('codeeko').get('endpoint'), '', 'Fresh profile must have no endpoint default.');
  ...
  while (Date.now() < readyDeadline) {
    const report = await vscode.commands.executeCommand<{ launches: { event: string; pid?: number }[][] }>('codeeko.exportStartupDiagnostics');
  ...
  assert.equal(await vscode.commands.executeCommand('codeeko.open'), true, 'Sidebar provider must initialize.');
  await vscode.commands.executeCommand('workbench.action.closeAuxiliaryBar');
  assert.equal(await vscode.commands.executeCommand('codeeko.open'), true, 'Refocusing must reuse the sidebar.');
  assert.equal(vscode.window.tabGroups.all.flatMap(group => group.tabs).filter(tab => tab.input instanceof vscode.TabInputWebview && tab.label === 'CodeEko').length, 0, 'Conversation must not occupy an editor tab.');
  ...
  const diagnostics = await vscode.commands.executeCommand<{ launches: { event: string }[][] }>('codeeko.exportStartupDiagnostics');
  ...
  await vscode.commands.executeCommand('codeeko.openSettings');
  const findSettings = () => vscode.window.tabGroups.all.flatMap(group => group.tabs).filter(tab => tab.input instanceof vscode.TabInputWebview && tab.label === 'CodeEko Settings');
  ...
  if (process.env.CODEEKO_HOST_UI_ONLY === '1') {
    await fs.writeFile(process.env.CODEEKO_HOST_REPORT!, JSON.stringify({ vscodeVersion: vscode.version, extensionActivation: true, sidebarInitializedAndRefocused: true, dirtyBufferPreserved: true }));
    return;
  }
  const live = await runLiveSmoke();
  const nativeReview = new NativeReview('codeeko-test-snapshot');
  let editing;
  try {
  ...
  assert.ok(process.env.CODEEKO_HOST_REPORT, 'Test launcher must provide a result path.');
  await fs.writeFile(process.env.CODEEKO_HOST_REPORT, JSON.stringify({ vscodeVersion: vscode.version, extensionActivation: true, commandsRegistered: true, blankEndpointDefault: true, sidebarInitializedAndRefocused: true, dirtyBufferPreserved: true, nativeDiffsOpened: true, live, editing, validation }));
}

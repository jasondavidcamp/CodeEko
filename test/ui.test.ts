...
test('extension commands, secure webview, discovery fallback, busy guard, cancellation and thread lifecycle', async t => {
  ...
  let undoApiCalls = 0;
  global.fetch = async () => { undoApiCalls++; throw new Error('Undo must not call a provider'); };
  settings.permissionMode = 'Review';
  await receive({ type: 'send', text: 'undo the pending changes' });
  assert.equal(sent.at(-1).thread.status, 'blocked');
  assert.equal(sent.at(-1).thread.undoTaskId, reviewId);
  assert.equal(await fs.readFile(path.join(root, 'pilot.ps1'), 'utf8'), 'function Get-UpdatedPilot {}');
  settings.permissionMode = 'Full access';
  approveUndo = true; await receive({ type: 'send', text: 'undo the pending changes' });
  assert.equal(undoApiCalls, 0);
  assert.equal(sent.at(-1).thread.messages.at(-2).content, 'undo the pending changes');
  assert.equal(await fs.readFile(path.join(root, 'developer.txt'), 'utf8'), 'keep staged work');
  assert.equal(await git(root, ['diff', '--cached']), stagedBeforeUndo);
  ...
  await receive({ type: 'send', text: 'undo the pending changes' });
  assert.match(sent.at(-1).thread.messages.at(-1).content, /no recorded edit task/);
  assert.equal(undoApiCalls, 0);

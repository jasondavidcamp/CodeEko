...
2. In VS Code, run **Extensions: Install from VSIX…** and select `llm-coding-agent-runtime-0.4.21.vsix`.
...
Ask for a focused code change to use editing. The agent reads files before proposing exact replacements. Stale content, unsaved editor buffers, and overlap with preexisting developer edits stop the task. Delete, move, and whole-file erasure open a preview and require approval, with Cancel selected initially. Completed changes open in native diff tabs. Use VS Code’s built-in Source Control to inspect and manage working-tree changes. Cancellation retains completed edits and reports them. Task-scoped undo remains implemented internally but is not exposed in the chat menu.
